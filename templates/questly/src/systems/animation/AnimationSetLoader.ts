...
export class AnimationSetLoader {
  ...
  private getBasePath(setName: string): string {
    ...
    // Use Mixamo animations for Quaternius characters
    if (setName === 'mixamo_enhanced') {
      return basePaths.mixamo_animations ||
             '/Assets/mixamo-animations';
    }
  ...
  async loadAnimationSet(setName: string): Promise<Record<string, THREE.AnimationClip>> {
    ...
    // Handle self-contained animations (use model's built-in animations)
    if ((animationSet as any).useBuiltInAnimations) {
      console.log(`[AnimationSetLoader] ${setName} uses built-in animations from model`);
      // Return empty object - the model's own animations will be used
      this.loadedSets.set(setName, {});
      return {};
    }
