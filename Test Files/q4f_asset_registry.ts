// src/systems/assets/AssetRegistry.ts
  ...
  private normalizeAdventurersAssets(data: any) {
    ...
    if (data.weapons?.melee) {
      data.weapons.melee.forEach((weapon: any) => {
        const id = `weapon_${weapon.name.toLowerCase().replace(/\s+/g, '_')}`;
        this.assets.set(id, {
          id,
          category: 'weapon',
          subcategory: 'melee',
          name: weapon.name,
          gltfPath: `${basePath}/${weapon.path}/gltf/${weapon.name}.glb`,
    ...
    if (data.shields) {
      data.shields.forEach((shield: any) => {
        const id = `shield_${shield.name.toLowerCase().replace(/\s+/g, '_')}`;
        this.assets.set(id, {
          id,
          category: 'item',
          subcategory: 'shield',
          name: shield.name,
          gltfPath: `${basePath}/${shield.path}/gltf/${shield.name}.glb`,
    ...
    if (data.items) {
      data.items.forEach((item: any) => {
        ...
        this.assets.set(id, {
          ...
          name: item.name,
          gltfPath: `${basePath}/${item.path}/gltf/${item.name}.glb`,
        ...
        if (Array.isArray(variants)) {
          variants.forEach((tree: any) => {
            ...
            this.assets.set(id, {
              ...
              subcategory: 'tree',
              name: tree.name,
              gltfPath: `${basePath}/${tree.path}/gltf/${tree.name}.glb`,
        ...
        if (Array.isArray(variants)) {
          variants.forEach((rock: any) => {
            ...
            this.assets.set(id, {
              ...
              subcategory: 'rock',
              name: rock.name,
              gltfPath: `${basePath}/${rock.path}/gltf/${rock.name}.glb`,
